...
export function applyChoice(stats: GameState['stats'], choice: Choice): GameState['stats'] {
  return {
    ...
    lgRelation: clamp(stats.lgRelation + choice.effects.lgRelation, 0, 100),
...
export function checkGameOver(stats: GameState['stats']): string | null {
  ...
  if (stats.lgRelation <= 0) return 'LG와의 관계가 파탄났습니다! 장학금과 지원이 끊겼습니다.';
...
export function applyEmergency(stats: GameState['stats']): GameState['stats'] {
  return {
    ...
    lgRelation: stats.lgRelation < EMERGENCY_THRESHOLD ? EMERGENCY_RECOVER_TO : stats.lgRelation,
...
export function calculateScore(week: number, stats: GameState['stats'], emergencyUsed: number): ScoreResult {
  ...
  const statSum = stats.satisfaction + stats.budget + stats.lgRelation + stats.academic;
  const statBonus = statSum * 5;
  const minStat = Math.min(stats.satisfaction, stats.budget, stats.lgRelation, stats.academic);
